function changeNavbarCurrent() {
  document
    .querySelector('.slider-navbar_cur-btn')
    .classList.remove('slider-navbar_cur-btn');
...
function switchSlideViaBar(clickedSlide) {
  if (clickedSlide != current && isSlideStop) {
    ...
    slides[clickedSlide].style.transform =
      'translateX(' + 100 * clickedMove + '%)';
    slides[current].style.transition = transitionValue;
    setTimeout(() => {
      slides[clickedSlide].style.transition = transitionValue;
    }, 0);
    setTimeout(() => {
